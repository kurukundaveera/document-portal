...
class DocumentIngestor:
        ...
        try:
            ...
            for uploaded_file in uploaded_files:
                ...
                if ext not in self.SUPPORTED_EXTENSIONS:
                    self.log.warning("Unsupported file type skipped", filename=uploaded_file.name)
                    continue
    ...
    def _create_retriever(self, documents):
        try:
            ...
            self.log.info("FAISS index created and saved to disk", path=str(self.session_faiss_dir), session_id=self.session_id)
            
            retriever = vector_store.as_retriever(search_type="similarity", search_kwargs={"k": 5})

            self.log.info("FAISS retriever created successfully and ready to use", session_id=self.session_id)
            return retriever
